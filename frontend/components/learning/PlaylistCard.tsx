  ...
  return (
    ...
    >
      <SpotlightCard
        ...
        className="rounded-[20px] sm:rounded-[24px] border border-slate-200/90 shadow-sm hover:shadow-xl hover:border-purple-300/80 transition-all duration-300 p-3.5 sm:p-6 flex flex-col justify-between group h-full"
      >
        <div>
          {/* ── Meta Row */}
          <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3.5 flex-wrap">
            ...
            {pl.source === "csv" ? (
              <span className="flex items-center gap-1 text-[9px] sm:text-[11px] font-bold text-purple-700 bg-purple-50 border border-purple-200/90 px-2 py-0.5 rounded-full ml-auto shadow-xs">
                <CheckCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-purple-600" /> CSV
              </span>
            ) : (
          ...
          <div className="space-y-1">
            <h3 className="text-xs sm:text-lg font-bold text-slate-900 group-hover:text-purple-600 transition-colors leading-snug line-clamp-2">
              {pl.title}
            </h3>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 sm:gap-3 pt-3 sm:pt-5 mt-3 sm:mt-4 border-t border-slate-100">
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => onWatch(pl)}
            className="w-full sm:flex-1 py-2 sm:py-2.5 rounded-full flex items-center justify-center gap-1.5 text-[11px] sm:text-xs font-bold text-white bg-gradient-to-r from-[#5f13e7] to-[#7928ca] hover:from-[#530dd4] hover:to-[#6b20b8] shadow-md shadow-purple-600/25 transition-all cursor-pointer"
          >
          ...
          {onSave && onUnsave && (
            <motion.button
              ...
              className={`w-full sm:flex-1 py-2 sm:py-2.5 rounded-full text-[11px] sm:text-xs font-bold border transition-all cursor-pointer ${
                isSaved
                  ? "text-purple-700 border-purple-300 bg-purple-50 hover:bg-purple-100 shadow-xs"
                  : "text-slate-700 border-slate-200/90 bg-slate-100/80 hover:bg-purple-50 hover:border-purple-300 hover:text-purple-700 shadow-xs"
              }`}
            >
              {isSaved ? "Saved ✓" : "Save"}
            </motion.button>

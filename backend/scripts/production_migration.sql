...
-- - Drops old RLS policies FIRST so ALTER COLUMN TYPE UUID succeeds without 0A000 error
...
-- --------------------------------------------------------------------
-- 3. DROP EXISTING RLS POLICIES FIRST (PREVENTS PG ERROR 0A000)
-- PostgreSQL requires policies referencing user_id to be dropped BEFORE changing column type.
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 4. SAFE DATA CLEANUP (PURGE STALE DEFAULTS / INVALID STRING IDs)
...
-- --------------------------------------------------------------------
-- 5. CONVERT USER_ID COLUMNS TO UUID (NOW SAFE AFTER DROPPING DEPENDENT POLICIES)
-- --------------------------------------------------------------------
...
BEGIN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'user_academic_profile' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'user_coding_profiles' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'user_progress' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'leetcode_progress' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'roadmap_progress' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'resume_scores' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'saved_playlists' AND column_name = 'user_id') != 'uuid' THEN
    ...
    IF (SELECT data_type FROM information_schema.columns WHERE table_name = 'video_progress' AND column_name = 'user_id') != 'uuid' THEN
        ALTER TABLE public.video_progress ALTER COLUMN user_id TYPE UUID USING user_id::UUID;
    END IF;
END $$;

-- --------------------------------------------------------------------
-- 6. REMOVE ORPHAN UUIDs NOT PRESENT IN AUTH.USERS
-- --------------------------------------------------------------------

DELETE FROM public.user_academic_profile WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.user_coding_profiles WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.user_progress WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.leetcode_progress WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.roadmap_progress WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.resume_scores WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.saved_playlists WHERE user_id NOT IN (SELECT id FROM auth.users);
DELETE FROM public.video_progress WHERE user_id NOT IN (SELECT id FROM auth.users);

-- --------------------------------------------------------------------
-- 7. SET NOT NULL & ADD FOREIGN KEY CONSTRAINTS TO AUTH.USERS(ID)
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 9. ENABLE RLS & RE-CREATE STRICT OWNERSHIP POLICIES
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 10. SECURE TRIGGER FUNCTION FOR NEW USERS
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 11. SECURE RPC FUNCTION: DERIVES IDENTITY INTERNALLY FROM AUTH.UID()
-- --------------------------------------------------------------------
